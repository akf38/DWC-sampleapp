...
describe "投稿のテスト" do
    let!(:list) { create(:list, title: "hoge", body: "hogehogebody") }
    ...
    describe "一覧画面のテスト" do
      ...
      context "一覧の表示とリンクの確認" do
        it "一覧表示画面に投稿されたもの表示されているか" do
          (1..5).each do |i|
            List.create(title: 'hoge'+i.to_s, body: 'hogehoge'+i.to_s)
          end
          visit todolists_path
          List.all.each_with_index do |list, i|
            ...
            expect(show_link.native.inner_text).to match (/todolists/i)
    ...
    describe "編集画面のテスト" do
      ...
      context "更新処理に関するテスト" do
        it "更新後のリダイレクト先は正しいか" do
          ...
          fill_in 'list[body]', with: Faker::Lorem.characters(number:20)
          click_button '保存'
          expect(page).to have_current_path update_todolist_path(list)
        end
      end
    end
end